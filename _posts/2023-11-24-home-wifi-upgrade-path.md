...
每个爱折腾的人都不会放弃折腾家里的网络的，无论是NAS，软路由，还是电视盒子，游戏机，WI-FI漫游等，甚至直接终极把家里整成一个机房，只要你想折腾就没有不能折腾的地方。

由于提到的NAS，软路由这些我都基本上折腾过了，本文主要聚焦的是  WI-FI 这一块，毕竟普通人最常最直接的体验就是 Wi-Fi 的顺畅与否。

### 0x01 蛮荒阶段
--------------------

从有上外网需求开始，家里经历过多个无线 AP 设备，从 K2 到 K2P，最开始从刷机 K2P 装某些插件实现上网，倒是稳定运行了很长一段时间。 后来搬家上了 NAS 后，随着硬盘越来越大，媒体资源越来越高清，K2P 的信号强度和 WI-FI 速度已经捉襟见肘，主要矛盾体现在日益增长的网络传输需求与羸弱的硬件传输能力的冲突。 
...
看着 TPLINK 路由器的硬件参数，无线都3000M/5000M的，再加上这个宣传的 “易展” 功能，我想着这不是直接起飞啊， 等到真正买回来才发现根本不是这么回事。
...
选择AP（有线中继）模式，IP地址和网关等全都不能手动设置，必须等自动分配，致于那个兼容模式，不明所以，开和关我也没感觉有啥区别。 

还有另外一种方法，关闭DHCP服务后，去lan口主动配置 IP，网关，子网掩码等设置，好歹是可以手动配置这个路由器的地址了。

后来我选择的是手动关闭 DHCP 服务来作为 AP，致于前面那一种有什么坑，我已经忘记了，反正不太行，也不想再为了写这篇文章去复现一下了，等想起来了再说。

![/styles/images/wifi_upgrade/tplink-web-ap2.png]({{ '/styles/images/wifi_upgrade/tplink-web-ap2.png' | prepend: site.baseurl }})

当然手动配置 LAN 口当然是好的，但是注意看图，图里只让你填了本机的 IP 地址和掩码，并没有让你选择网关和DNS服务器，

如果你不熟悉网络的话可能不了解这代表什么，稍微了解一点就知道这两个选项对于能不能上网至关重要。

我不知道 TPLINK 的开发者是怎么想的，实际上缺失了这两个配置，导致我接入WI-FI的设备无法正确设置 DNS 。 我的软路由作为网关，最后的 IP 地址并不是 .1，而是 .2（历史原因），我甚至怀疑是 TPLINK 默认认为网关地址是 .1 结尾，而自动根据 LAN 口设置配置了，反正在我的环境中出现各种问题。
...
这里我的环境相对来说比较简单，网关和 DHCP 服务器好歹还是同一个，如果你有旁路由这样的二级网关，那估计踩的坑会更多。
...
结果光看斗鱼就经常缓存，不知道是掉线还是什么，我千兆的网清晰度连个蓝光都看不了，坑爹。
...
我用各种测速软件测出来的结果很美好，但实际体验太差劲了，只能说一分钱一分货。


### 0x03 近乎完美的 AP 设备
--------------------

今年双11，我痛下决心下血本买了两个 Unifi 的 U6 mesh 可乐罐，准备用来替换家里的 TPLINK。
...
Unifi 的 AP 设备，硬件上还是存在一点点不足，U6 mesh 可乐罐发热严重，直接放置的话不算很稳容易倒。 我感觉是过度追求外观了损失了部分散热甚至是寿命。 当然目前暂时还没有发现因为散热导致的卡顿，但我也怀疑如果长期在高温的情况下寿命能否长久。

其次，1500左右的 AP 设备，居然还是千兆，这个价格和这个配置，不得不说略显落后。 据说接下来很快会出 2.5G 的 AP，但考虑到它的价格，千兆也不是不能凑合。

另外，Unifi 全家桶还有其他的设备，包括网关交换一体机，功能也十分强大，和这个 AP 可以统一配置，就是价格实在是承受不起，当然这倒不是他们硬件的不足，主要是我的资金不足。

#### 自建 Unifi controller 遇到的一点小 BUG (Docker)
